...
void main() {
  group('markdown stream collector', () {
    ...
    test(
      'pushMarkdownDelta preserves paragraph breaks from double newline',
      () {
        final now = DateTime.utc(2026, 2, 22, 4, 0, 0);
        const state = MarkdownStreamCollectorState();
    ...
    test(
      'does not soft-flush a table row to prevent breaking table syntax',
      () {
        final now = DateTime.utc(2026, 2, 22, 4, 0, 0);
        final state = MarkdownStreamCollectorState(
          pendingBuffer:
    ...
    test(
      'streaming table rows produce contiguous lines for markdown parsing',
      () {
        final now = DateTime.utc(2026, 2, 22, 4, 0, 0);
        const state = MarkdownStreamCollectorState();
        final deltas = [
    ...
    test('does not soft-flush an incomplete markdown image link', () {
      final now = DateTime.utc(2026, 2, 22, 4, 0, 0);
      final state = MarkdownStreamCollectorState(
        pendingBuffer: '![Imagen de prueba](https://placehold.',
        pendingSince: now.subtract(const Duration(milliseconds: 300)),
      );
      final result = maybeFlushSoftChunk(state, now: now);
      expect(result.chunk, isNull);
    ...
    test('does not soft-flush an incomplete inline link', () {
      final now = DateTime.utc(2026, 2, 22, 4, 0, 0);
      final state = MarkdownStreamCollectorState(
        pendingBuffer: 'Visita [OpenAI](https://openai.',
        pendingSince: now.subtract(const Duration(milliseconds: 300)),
      );
      final result = maybeFlushSoftChunk(state, now: now);
      expect(result.chunk, isNull);
    ...
    test(
      'pushMarkdownDelta sets pendingStartsAfterNewline when newline splits buffer',
      () {
        final now = DateTime.utc(2026, 2, 22, 4, 0, 0);
        const state = MarkdownStreamCollectorState();
    ...
    test(
      'pushMarkdownDelta preserves pendingStartsAfterNewline across no-newline deltas',
      () {
        final now = DateTime.utc(2026, 2, 22, 4, 0, 0);
        const state = MarkdownStreamCollectorState();
    ...
    test(
      'pushMarkdownDelta does not set pendingStartsAfterNewline without newline',
      () {
        final now = DateTime.utc(2026, 2, 22, 4, 0, 0);
        const state = MarkdownStreamCollectorState();
    ...
    test(
      'maybeFlushSoftChunk remaining state has pendingStartsAfterNewline false',
      () {
        final now = DateTime.utc(2026, 2, 22, 4, 0, 0);
        final state = MarkdownStreamCollectorState(
          pendingBuffer:
