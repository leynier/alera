...
void _registerClaudeRuntimeCcsTests(
  ...
  test('installs status hooks into CCS instance local settings, not user Claude settings', () async {
    final home = readHome();
    ...
    Link(p.join(instanceDir.path, 'settings.json'))
  ...
  test('reports partial when runtime is ready but CCS instance local hooks are missing', () async {
    final home = readHome();
    final service = readService();
    final preparation = await service.prepareForTerminalLaunch();
    final instanceDir = Directory(
