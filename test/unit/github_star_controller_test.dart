...
void main() {
  group('GitHubStarController', () {
    test(
      'refresh hides the prompt when star status cannot be resolved',
      () async {
        ...
        final controller = container.read(
          gitHubStarControllerProvider.notifier,
        );
    ...
    test('star does nothing while the controller is still loading', () async {
      ...
      final service = _FakeGitHubStarService(
        checkStarredFuture: completer.future,
      );
      final container = ProviderContainer(
        overrides: [gitHubStarServiceProvider.overrideWithValue(service)],
      );
    ...
    test(
      'refresh can expose not-starred and star can promote to starred',
      () async {
        final service = _FakeGitHubStarService(
...
class _FakeGitHubStarService extends GitHubStarService {
  ...
  Future<bool?> checkStarred() async {
    if (checkStarredFuture case final Future<bool?> future) {
      return future;
    }
    return checkStarredResult;
  }
