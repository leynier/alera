...
void registerRunBoardTerminalFocusTests() {
  for (final returnToWorkspace in [false, true]) {
    testWidgets(
      returnToWorkspace
          ? 'Return to Workspace restores retained terminal keyboard focus'
          : 'Open Terminal restores retained terminal keyboard focus',
      (tester) async {
        ...
        final workspace = returnToWorkspace
            ? seed.activeWorkspace!
            : seed
                  .workspacesFor('project-1')
                  .singleWhere(
                    (workspace) => workspace.id == 'workflow-attempt-2',
                  );
        final session = runtime.sessionFor(
          workspace: workspace,
          tab: seed.tabsFor(workspace.id).single,
        );
        ...
        try {
                    ...
                    builder: (context, ref, _) {
                      ...
                      return Stack(
                        ...
                        children: [
                          ...
                          if (visible)
                            RunBoardPage(onReturnToWorkspace: returnFromBoard),
                        ],
                      );
                    },
                  ),
          ...
          final navigation = container.read(
            runBoardNavigationProvider.notifier,
          );
          navigation
            ..open()
            ..selectRun('run-1')
          ...
          await tester.tap(
            find.text(
              returnToWorkspace ? 'Return to Workspace' : 'Open Terminal',
            ),
          );
          await tester.pumpAndSettle();
