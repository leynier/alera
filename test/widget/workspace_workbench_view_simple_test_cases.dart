...
void _registerSimpleWorkbenchViewTests() {
  testWidgets('Simple mounts only its selected surface without a tab strip', (
    tester,
  ) async {
    final tabs = [
    ...
    Future<void> pump({required bool simple, String? id}) => _pumpWorkbenchView(
      tester,
      tabs: tabs,
      layout: layout,
    ...
    await pump(simple: true, id: 'primary');
    ...
    await pump(simple: false);
    expect(find.byKey(const ValueKey('terminal-primary')), findsOneWidget);
    expect(find.byKey(const ValueKey('terminal-aux')), findsOneWidget);
    expect(identical(terminalRuntime.peekSession('primary'), primary), isTrue);
