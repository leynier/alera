...
If a workspace record names an SSH host, quota lookups go through `alera runtime-proxy` on that host's installed sidecar so credentials stay there. This does not mean Alera can create or open a managed remote worktree yet.
