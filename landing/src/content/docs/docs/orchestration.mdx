...
Profiles live in **Settings → Agent Profiles**. Each one has a name, an adapter from the built-in registry (`codex`, `claude`, `copilot`, `cursor`, `agy`, `opencode`, `opencode2`, `pi`, `amp`, `grok`, `fx`, and related defaults), a launch configuration, a description used as a routing signal, and an optional quota group.

```bash
alera orchestration agent-profiles --json
```

Only you create or edit profiles. A coordinator picks from that closed catalog; it never invents a launch command.
