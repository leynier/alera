...
SSH worktrees (run the same model on a remote machine) are on the roadmap. They are not part of the current workbench. **Settings → Remote Hosts** and `alera ssh-target bootstrap` install a runtime sidecar only. `alera workspace register --host-id` is metadata only and does not create a remote worktree.
