...
#[test]
fn promptless_profile_launch_starts_the_command_without_delivery_payload() {
    let dir = tempfile::tempdir().unwrap();
    let marker = dir.path().join("promptless-launch.txt");
    let recorder = write_recorder(
        dir.path(),
        "record-promptless-launch.sh",
        &format!("printf X >> {}; sleep 30", marker.display()),
    );
    let token = "promptless-profile-launch-token";
    ...
    send(
        ...
        json!({"id": 10, "type": "project.upsert", "payload": {
            "id": "project-1", "name": "Promptless Project",
    ...
    send(
        &mut writer,
        json!({"id": 1, "type": "workspace.upsert", "payload": workspace_payload("promptless-workspace", dir.path())}),
    );
    assert_eq!(read_response(&mut reader, 1)["ok"], json!(true));
    send(
        &mut writer,
        json!({"id": 2, "type": "agentProfile.upsert", "payload": {
            "name": "Promptless Profile", "agentType": "codex",
    ...
    send(
        ...
        json!({"id": 3, "type": "agentProfile.launchIdempotent", "payload": {
            "workspaceId": "promptless-workspace", "profileId": profile_id,
            "clientMutationId": "promptless-launch-1"
        }}),
    );
    let omitted = read_response(&mut reader, 3);
    assert_eq!(omitted["ok"], json!(true), "{omitted}");
    wait_for_file(&marker);
    let omitted_tab_id = omitted["payload"]["tab"]["id"]
        .as_str()
        .unwrap()
        .to_string();
    assert!(omitted["payload"]["tab"]["payload"]
        .get("initialPrompt")
        .is_none());
    assert!(omitted["payload"]["tab"]["payload"]
    ...
    send(
        &mut writer,
        json!({"id": 4, "type": "agentProfile.launchIdempotent", "payload": {
            "workspaceId": "promptless-workspace", "profileId": profile_id,
            "prompt": "", "clientMutationId": "promptless-launch-2"
        }}),
    );
    let empty = read_response(&mut reader, 4);
    assert_eq!(empty["ok"], json!(true), "{empty}");
    assert!(empty["payload"]["tab"]["payload"]
        .get("initialPrompt")
        .is_none());
    assert!(empty["payload"]["tab"]["payload"]
        .get("pendingAgentPrompt")
        .is_none());
    let empty_tab_id = empty["payload"]["tab"]["id"].as_str().unwrap().to_string();
    ...
    runtime.block_on(async {
        let store = alera_core::runtime::RuntimeStore::open(dir.path())
            .await
            .unwrap();
        let omitted_tab = store
            .find_workspace_tab(&omitted_tab_id)
            .await
            .unwrap()
            .unwrap();
        assert!(
            omitted_tab.payload.get("initialPrompt").is_none()
                || omitted_tab.payload["initialPrompt"].is_null()
        );
        assert!(
            omitted_tab.payload.get("pendingAgentPrompt").is_none()
                || omitted_tab.payload["pendingAgentPrompt"].is_null()
        );
        let empty_tab = store
            .find_workspace_tab(&empty_tab_id)
            .await
            .unwrap()
            .unwrap();
        assert!(
            empty_tab.payload.get("initialPrompt").is_none()
                || empty_tab.payload["initialPrompt"].is_null()
        );
        assert!(
            empty_tab.payload.get("pendingAgentPrompt").is_none()
                || empty_tab.payload["pendingAgentPrompt"].is_null()
        );
    });
}
