...
fn mobile_allowlist_includes_workspace_mutations() {
    ...
    assert!(mobile_request_allowed("workspace.handOff"));
    assert!(mobile_request_allowed("workspace.handOn"));
...
fn account_and_push_capabilities_are_additive_and_not_mobile_admin_verbs() {
    ...
    assert!(
        MOBILE_HELLO_CAPABILITIES.contains(&RUNTIME_HOST_AI_ASSIST_WORKSPACE_IDENTITY_CAPABILITY)
    );
    assert!(MOBILE_HELLO_CAPABILITIES.contains(&RUNTIME_HOST_AI_ASSIST_SPEECH_MESSAGE_CAPABILITY));
    assert!(
