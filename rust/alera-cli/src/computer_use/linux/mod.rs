...
impl LinuxProvider {
    ...
    /// Observe one window through an existing connection.
    ///
    /// Shared with the action path, which has to hand back a fresh observation:
    /// the agent chooses its next element index from the reply, so an action that
    /// returned the pre-action tree would be handing out indexes it just
    /// invalidated.
    async fn observe(
        &self,
        ...
        let (root, window) = session
            .read_window(app, window_index, DEFAULT_BUDGET.max_nodes)
            .await?;
        ...
        Ok(Snapshot {
            ...
            screenshot: None,
            screenshot_error: include_screenshot.then(|| screenshot_detail(self.session)),
...
impl ComputerUseProvider for LinuxProvider {
    ...
    async fn act(&self, request: ActionRequest<'_>) -> ComputerResult<ActionOutcome> {
        ...
        // callers cannot interleave into a sequence neither of them asked for.
        let _gate = action_gate::hold().await;
        let session = self.connect().await?;
        let window = session.window_proxy(request.app, window_index).await?;
        let target = session.node_at_path(&window, &element.path).await?;
        at_spi_action::ensure_still_matches(&target, &element).await?;
        let performed = match &request.target {
            ActionTarget::Click => at_spi_action::click(&target).await?,
            ActionTarget::SetValue { value } => at_spi_action::set_value(&target, value).await?,
            ActionTarget::PerformAction { action } => {
                at_spi_action::perform_named(&target, action).await?
            }
        };
        // Re-read through the same connection so the agent's next index comes
        // from after the action rather than before it.
        let snapshot = self
            .observe(
                &session,
                request.app,
        ...
        // A window that closed under a successful action is reported as changed,
        // not as a failure: the action happened, and a failure would invite a
