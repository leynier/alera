...
pub struct RuntimeHostRpcClient {
...
impl RuntimeHostRpcClient {
    pub async fn connect(runtime_dir: &Path) -> Result<Option<Self>> {
        Self::connect_with_capability(runtime_dir, None).await
    }

    pub async fn connect_mobile(runtime_dir: &Path) -> Result<Option<Self>> {
        Self::connect_with_required_capability(runtime_dir, RUNTIME_HOST_MOBILE_CAPABILITY).await
    }

    pub async fn connect_with_required_capability(
        runtime_dir: &Path,
    ...
    pub async fn connect_or_start(runtime_dir: &Path) -> Result<Self> {
    ...
    pub async fn connect_or_start_persistent(runtime_dir: &Path) -> Result<Self> {
        persistence::connect_or_start_persistent(runtime_dir).await
    }

    pub async fn connect_or_start_mobile(runtime_dir: &Path) -> Result<Self> {
        Self::connect_or_start_with_required_capability(runtime_dir, RUNTIME_HOST_MOBILE_CAPABILITY)
            .await
    }

    pub async fn connect_or_start_with_required_capability(
        runtime_dir: &Path,
        required_capability: &str,
    ) -> Result<Self> {
        if let Some(client) =
    ...
    pub async fn request<T, P>(&mut self, request_type: &str, payload: &P) -> Result<T>
    where
        T: DeserializeOwned,
        P: Serialize + ?Sized,
    {
    ...
    pub async fn request_value_with_deadline<P>(
        &mut self,
        request_type: &str,
        payload: &P,
        deadline_ms: u64,
    ...
    pub async fn request_value<P>(&mut self, request_type: &str, payload: &P) -> Result<Value>
    where
        P: Serialize + ?Sized,
    {
