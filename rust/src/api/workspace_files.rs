...
fn entry_for_path(
    ...
    let has_children_hint = if matches!(kind, WorkspaceFileKind::Directory) {
        if hide_ignored {
            match hide_ignored_directory_kind(path) {
                HideIgnoredDirectoryKind::Listable | HideIgnoredDirectoryKind::Unknown => true,
                HideIgnoredDirectoryKind::Empty => false,
                HideIgnoredDirectoryKind::IgnoredOnly => return Ok(None),
            }
        } else {
            has_unprotected_child(path).unwrap_or(false)
        }
    } else {
        false
    };
...
fn has_unprotected_child(directory: &Path) -> Result<bool, WorkspaceFileError> {
    for result in fs::read_dir(directory)
        .map_err(|error| WorkspaceFileError::from_io(error, directory.to_string_lossy()))?
    {
        let entry = result
...
fn hide_ignored_directory_kind(directory: &Path) -> HideIgnoredDirectoryKind {
