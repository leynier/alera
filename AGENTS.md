...
- The `release` flavor preserves the historical identifiers (`dev.leynier.alera`, `alera`, `Alera`). It MUST be selected by CI for any artifact intended to be installed by an end user. `.github/workflows/desktop-build.yml` and `.github/workflows/release-cut.yml` set `ALERA_FLAVOR: release` at the job env level.
