...
- `deferred_delivery_cases::coordinator_promotion_waits_for_deferred_delivery` fails intermittently under the parallelism of a full `make rust-test` and passes in isolation. Re-run it on its own before treating a red `rust-test` as a real regression, and do not chase it as fallout from an unrelated change.
