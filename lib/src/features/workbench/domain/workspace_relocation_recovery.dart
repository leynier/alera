...
class const WorkspaceRelocationRecoverySnapshot({
  ...
  factory fromResponse(Workspace workspace, Object? response) {
    ...
    _requireIdentity(workspace, Workspace.fromJson(_map(report['workspace'])));
    ...
    if (owner != null) {
      ...
      _requireIdentity(
        workspace,
        Workspace.fromJson(_map(owner['workspace'])),
        ownerLocal: true,
      );
      for (final raw in _list(owner['items'])) {
        ...
        _requireIdentity(
          workspace,
          Workspace.fromJson(_map(journal['source'])),
          ownerLocal: true,
        );
        _requireIdentity(
          workspace,
          Workspace.fromJson(_map(journal['destination'])),
          ownerLocal: true,
        );
    ...
    for (final raw in _list(report['homeIntents'])) {
      ...
      final source = Workspace.fromJson(_map(retained['source']));
...
WorkspaceRelocationRecoveryEntry _localEntry(
  ...
  final journal = _map(item['relocation']);
  final source = Workspace.fromJson(_map(journal['source']));
  final destination = Workspace.fromJson(_map(journal['destination']));
  _requireIdentity(workspace, source);
