...
| WS-003 | Clone | Local bare remote success | Clone controlled remote using dialog | Progress, destination, project registration and cancellation agree | en curso | pendiente | Native Flutter and GPUI cloned the same local bare into separate disposable destinations, both HEAD1915de8 and registered once with correct names/paths, then showed Welcome and Project cloned. Progress was too brief to capture; initial mode focus fails in GPUI (F039). Error/cancel/release pending. |
...
| WS-022 | Clone ownership | Competing destination, failure/cancel/restart | Isolated filesystem regressions before any live race | A failed job never removes another job or user's destination | corregido sin revalidar | pendiente | F040 original sentinel test failed before fix. Per-job owned staging/exclusive publish implemented;11 directed macOS tests pass, including real Git workers and competing publishes. Live host still old, release/lifecycle and Linux/Windows execution pending. No live destructive race attempted. |
