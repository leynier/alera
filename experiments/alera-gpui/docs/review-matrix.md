...
| ID | Component | Scenario | Actions | Expected | D | R | Observed / evidence |
| --- | --- | --- | --- | --- | --- | --- | --- |
| DS-001 | Inputs | Empty, filled and invalid values | Type/select/replace/cancel; blur; submit | Label, placeholder, focus, validation and value match | en curso | pendiente | Rename native empty/filled/error/typing and Enter now work; generic label notch regression failed21px versus outline20px and passes after border-inset correction. Other fields/precise visual comparisons and release pending. |
| DS-002 | Textareas | Multiline Unicode and selection | Paste multiline accents/emoji; select; undo/redo | No UTF range panic, newline loss or bad selection | en curso | pendiente | Native Automation description preserves three-line Unicode through create/reopen/failed-save/retry; fixed3/5-row geometry tested. Plain prompt gray selection works in2078232e. Remaining controls, undo/redo, long/native clipboard and release pending. |
...
| DS-004 | Dropdowns | Trigger and click-outside | Open, toggle same trigger, click outside | Arrow differentiates input, single overlay closes correctly | en curso | pendiente | Text Actions and Automation selectors exercised natively, including external close and selection without closing parent. Material triangle/full-width menu added for Automation. Other controls, viewport limits, precise padding and release remain pending. |
| DS-005 | Dropdowns | Search, keyboard and disabled option | Filter list; arrows/Enter/Escape; unavailable value | Correct selection, no lost focus or enabled invalid option | en curso | pendiente | Native Automation Up/Enter selection, focus restoration/reopen and menu-only Escape pass; Text Action agent/model menus exercised. Search, disabled/long cases and remaining selectors/release still pending. |
...
| DS-010 | Overlays | Nested and near viewport edges | Open nested menu/popover/dialog; Escape/outside | No clipping, click-through, duplicate IDs or focus trap | en curso | pendiente | Real modal selection and nested outside regressions reproduced/fixed; native2078232e only closes top editor and keeps Project selection inside its parent. Other overlay families/viewport limits and release pending. |
...
| ID | Component | Scenario | Actions | Expected | D | R | Observed / evidence |
| --- | --- | --- | --- | --- | --- | --- | --- |
| WS-001 | Project add | Existing local repository | Register fixture repo using UI | Correct project/root workspace and duplicate handling | en curso | pendiente | Registration and duplicate Beta re-add pass in both clients without extra project/workspace. Both expand previously collapsed Beta and show Welcome. Native Welcome New Workspace chooses first Alpha, sidebar chooses active Beta. F-005 visual precision/variants and F-017 draft/clone/release remain tracked independently. Evidence F017-*. |
...
| WS-003 | Clone | Local bare remote success | Clone controlled remote using dialog | Progress, destination, project registration and cancellation agree | en curso | pendiente | Profile C native Flutter/GPUI both clone HEAD0d7dd128 with sidecar6f44ec08, one registration per destination and no staging leftovers. F039 mode autofocus works both directions. Flutter transient progress captured; GPUI clone finishes before capture, so sustained spinner comparison remains open. Evidence C-*-clone-* under .dart_tool/gpui-review-20260830.4ve14G/evidence. |
| WS-004 | Clone | Failure/cancel/retry | Use invalid destination/remote; cancel/retry | No stuck busy state or orphan fixture registration | en curso | pendiente | Profile C invalid local origin fails in both clients; final destination/staging absent, project count stays2. Both dismiss and recover; Cancel before submit leaves inventory unchanged. Flutter AX duplicates its error (REF-004), unlike GPUI single toast. Existing-destination rejection passed historically in B. Corrected retry to the same failed destination and release remain pending. |
| WS-005 | Workspace create | Manual branch and parent selectors | Create child workspace from selected source | Branch/path/parent correct and selectors keyboard accessible | fallo | pendiente | C native step1 confirms F041 path filtering, order, overflow and radio/segment geometry. Source correction and two directed tests pass, native replay pending. Flutter empty Existing Branch uses manual source input; GPUI fallback and Create Another reset require follow-up. No manual workspace submitted yet. |
...
| WS-009 | Grouping | Project | Expand three-level tree and multiple projects | Counts, nesting, row geometry and state correct | en curso | pendiente | C root/Review Child/Review Grandchild created by UI and shared across clients. Full collapse/spacing comparison pending. F044 preserves Project collapse during search; regression passes, visual replay blocked by locked Mac. |
| WS-010 | Grouping | None | Switch to None on same fixtures | No inherited project padding or missing descendants | en curso | pendiente | F044 native R1 None/Recent and global pins now match Flutter; collapsed regular parent keeps pinned child visible. All/row geometry and identical-name tie ordering require follow-up. |
| WS-011 | Sorting | Name/activity/custom choices | Apply every exposed ordering | Stable hierarchy and correctly ranked descendant activity | en curso | pendiente | F044 tests cover Recent grouped vs flat and pinned Name/Activity grouping rules. All exposed sort choices/native activity fixtures still pending. |
| WS-012 | Filtering | Search/project/tag/active filters | Combine, clear and restore filters | Correct rows/counts; selection remains coherent | en curso | pendiente | F044 native R1 search Review Child preserves collapsed Project/All and promotes matching child when parent is filtered; Flutter comparison captured. Both queries cleared. Combined tag/active/project-selection scenarios pending. |
| WS-013 | Pinned | Pin/unpin and repeat pinned | Toggle pins and repeat option; relaunch | Correct duplicates, ordering and persistence | en curso | pendiente | R1 native three pins, None/Recent global ordering, regular parent collapse, Repeat Pinned off/on and cross-client synchronization match. Activity/relaunch and visual geometry pending; tests cover Activity projection only. |
...
| WS-017 | Workspace actions | Rename/reveal/copy/open | Use each context action on fixture | Correct identity, target, toast and keyboard dismissal | en curso | pendiente | F047 native initial typing, blank validation, error clearing and Enter submit pass after fix. Root-backed tests catch the additional Tab escape; native fc535944 now Tab focuses Cancel and Enter closes without changing sidebar. Final AX dialog-container repair, reference native Tab (tool-rejected attempt), other context actions and release remain open. |
...
| WS-021 | New Workspace | Dialog visuals and entry-point context | Open from Welcome/sidebar/project menu with different active project | Matching surface, spacing, inputs, defaults and dismissal | fallo | pendiente | F-017 entry defaults now match Welcome=first/Sidebar=active. F-022 visual mismatch confirmed in F017-*-sidebar-project-context; dictation microphone is intentionally excluded. |
| WS-022 | Clone ownership | Competing destination, failure/cancel/restart | Isolated filesystem regressions before any live race | A failed job never removes another job or user's destination | en curso | pendiente | F040 original sentinel regression failed before fix. Thirteen directed macOS tests pass, including real Git workers, exclusive publish, Stop/Restart busy guards and cancelling-job retention. Profile C UI Start launched verified sidecar6f44ec08; native success/failure cleanup passes in Flutter and GPUI. Release GUI/restart recovery pending; Linux/Windows source-only. No live destructive race attempted. |
| WS-023 | Manual source branch | Empty branch catalog/error fallback | Switch to Existing Branch when all local branches are in use; recover catalog error | Editable source field with correct validation/retry, not an unusable empty picker | en curso | pendiente | Native GPUI SHA30e99974 now matches the empty catalog input, focuses it and accepts main without another click. Continue carries main into Settings. C-WS023-* evidence; retry/error and release still pending. |
| WS-024 | Create Another | Successful manual workspace followed by another | Enable Create Another, submit fixture, inspect next draft | Manual settings step stays open, resets fields/parent and reloads branches; created workspace remains accessible | validado | pendiente | F042 SHA93661431 created Review Completion with Terminal1, kept Step2 empty/No Parent/checkbox on, and next-draft typing reached the branch field. Escape discarded the draft, no branch/workspace created for it. Native pwd confirms the fixture workspace cwd. Flutter prior C-WS024 reference behaves equivalently. Setup/busy variants tracked separately; release pending. |
| WS-025 | Manual creation | Setup overlap and busy form transitions | Use controlled Setup command; attempt mode/back/reopen while create is pending | Initial normal terminal not lost, no duplicate job, form identity/draft preserved | en curso | pendiente | C-WS025-trace-1 proves Terminal1 before active Setup; autoclose preserves Terminal1. Controlled real-bridge tests prove busy/reentry/error/retry without duplicate requests. Deliberately slow native create/busy controls and release remain pending. Override restored. |
| WS-026 | Workspace dialog requests | Reordered, late and dropped-owner responses | Retain/reorder replies through controlled RuntimeBridge peer | No stale branch/profile replacement, late error or owner resurrection | en curso | pendiente | F043 five integrated request tests and twelve combined manual tests pass; stale branch/error regressions failed before fix. Build7742e1ed native smoke blocked by locked Mac. |
| WS-027 | Workspace tree recovery | Collapsed stale relation cycle | Render controlled invalid relation graph | Keep a recoverable root without exposing normally collapsed descendants | en curso | pendiente | F044 cycle regression failed with0 rows, then passed with1; filtered-parent and normal collapsed subtree assertions pass. No runtime metadata corrupted for this test. |
...
| FS-007 | Replace | One/all/conflict/cancel | Replace fixture matches with changed-file conflict | Accurate preview, confirmations and safe writes | en curso | pendiente | F-020 inactive dirty guard matches Flutter. Native clean cached reload failed once, corrected and revalidated. F-023 per-match button now visible/clickable with two-line preview; both clients replace/reload controlled Unicode fixture, restored to baseline. Replace all/conflict/cancel/delayed variants still pending. |
...
| FS-012 | Editor | Close/reopen with pending read/save | Race file load/save against tab/workspace switch | No resurrected tabs, lost drafts or stale content | en curso | pendiente | F-019 model tests cover delayed/reversed completion, closed/reopened identity, duplicate replies and later edits. Native debug confirms Unicode reveal, cached draft/undo preservation and Save/navigate/Undo/Save baseline consistency. Actual delayed runtime/close-in-flight and release are not validated. |
...
| FS-014 | Explorer Save All | Two homonymous dirty files across workspaces and no-op | Edit A/B, invoke Save All from B, repeat with no changes, undo both and Save All | Both intended files saved once, correct count, no-op feedback and clean restoration | validado | pendiente | F-021 native Flutter and GPUI report Saved 2 editor files and both disk files match their distinct drafts. GPUI no-op reports No unsaved editor files; Undo across workspace banks then Save All restores both baselines. Evidence F021-*-save-all-two-workspaces and F021-gpui-save-all-noop/restored. |
| FS-015 | Explorer Save All | Read errors, conflicts, concurrent writes and close/reopen | Inject errors/delays and change target identity mid-batch | No overwrite or save on replaced entity; error scoped; busy state recovers | pendiente | pendiente | Target/entity and failed-read model safeguards exist; native delayed/error variants not run. |
...
| SC-002 | Views | Tree/list and grouped/unified | Switch view modes and expand directories | Stable selection and scoped row actions | en curso | pendiente | F045 GPUI wrote list instead of Flutter flat and read flat as tree. Rust and Flutter Drift regressions failed before fix, then pass with canonical shared fixture. Storage migration preserves other/unknown fields and concurrent updates. Native toggle/restore and grouped/unified UI remain pending while Mac locked. |
...
| PR-007 | Provider-not-detected state | Header, Refresh and centered message | Open local remote fixture, click Refresh | Header remains available and unavailable state matches Flutter | validado | pendiente | F010-gpui-provider-fixed / provider-refresh match F010-flutter-provider-recheck on debug 0e0bc9d6; paired panel crops saved. |
...
| AI-007 | Text Actions | CRUD coverage overview | Follow detailed scenarios AI-010 through AI-018 | Stable identity, proper overrides and list persistence | en curso | pendiente | Split into action-level scenarios after F-006/F-024 findings; a visual empty-state pass does not close CRUD coverage. |
...
| AI-010 | Text Actions catalog | Empty master/detail | Open empty catalog in both clients | Full-height divider, New Action label, matching centered states, no extra Reset | validado | pendiente | F006-gpui-empty-after matches F006-flutter-empty-recheck in debug. |
| AI-011 | Text Actions create | New definition and required fields | Create disposable definition, test empty fields | Enabled/agent/model/reasoning available before Save; errors visible | en curso | pendiente | Native Beta created with disabled/Codex/gpt-5.4/medium, runtime payload and Flutter cold restore match. Blank Name error visible. Empty Prompt/duplicate-name variants pending. |
| AI-012 | Text Actions edit | Edit name/prompt without losing overrides | Modify stored action with reasoning for multiple models | Unedited values preserved and correct target updated | en curso | pendiente | reasoning_after_edit regression passes; native persistence/reload not yet run. |
| AI-013 | Text Actions overrides | Inheritance and model transitions | Select global/agent/model/reasoning values | Valid catalogs, inherited labels and per-model settings retained | en curso | pendiente | Native agent Down/Enter and explicit model/reasoning persisted. Outside click closes menu. F026 discovery order/default and menu anchor follow-up in source; full transitions pending. |
| AI-014 | Text Actions duplicate | Copy name, fields and selection | Duplicate action twice with Unicode names | Unique names, copied settings and matching selected draft | en curso | pendiente | Native Beta Copy is inserted, selected and seeded with exact prompt/disabled/Codex/gpt-5.4/medium. Repeated/Unicode naming variants pending. |
| AI-015 | Text Actions availability | Enable/disable from list and editor | Toggle both controls and reopen | Correct draft versus persisted semantics and menu availability | pendiente | pendiente | New-action Enabled control missing in old GPUI. |
| AI-016 | Text Actions ordering | Drag reorder and persistence | Reorder long fixture list | Stable identity, correct insertion, persisted order | en curso | pendiente | Native drag moves Beta Copy from third to first with selected draft preserved. Long-list/auto-scroll/reverse/preview behavior and restore pending. |
| AI-017 | Text Actions deletion | Confirm/cancel and target identity | Inspect confirmation; test authorized fixture completion | Explicit confirmation and no deletion of a changed selection | en curso | pendiente | Both confirmation dialogs compared and cancelled natively; no deletion performed. Epoch regression passes. Final native Delete requires action-time confirmation; AX dialog role/text remains absent in GPUI capture. |
| AI-018 | Text Actions lifecycle | External changes, switching and reopen | Delay persistence and change selected action | No stale values overwrite another action or resurrect a deleted one | pendiente | pendiente | Removed-action save guard added; full draft/lifecycle ownership pending. |
...
| SE-004 | Persistence | Change/reopen/relaunch/cross-client | Modify controlled preferences and reload both clients | Same saved values without lost unrelated fields | en curso | pendiente | F045 format/migration and F046 first-load/fallback/stale-get/local-before-remote tests pass. Shared Flutter/Drift fixtures preserve local fields. Native change/reopen/relaunch comparison is blocked by locked Mac; no blanket persistence acceptance. |
...
| SE-011 | Workbench preferences | First desktop initialization, offline fallback, delayed load and save | Deliver controlled get/save responses and compare Flutter repository | Local choices seed new runtime; old reads do not revert newer choices; local save precedes remote event | en curso | pendiente | F046 six real-bridge tests pass; five failed before their fixes. Flutter repository/Drift combined6 tests pass and storage migration2 tests pass. Native/release gate remains blocked by Mac lock. Rapid overlapping saves beyond this local/remote ordering are not claimed. |
...
| OM-006 | Canvas catalog | Empty and Active/History availability | Open empty catalog; introduce history fixtures | Full-width empty state and disabled filter without history | en curso | pendiente | Empty/full-width/disabled Active revalidated in native debug 0e0bc9d6, matching Flutter; populated history fixture pending runtime capability correction. |
| OM-007 | Canvas selection | Pins, waiting/live/history and explicit selection | Populate mixed states, toggle filter and navigate | Correct groups; hidden history does not silently change explicit selection | en curso | pendiente | Pure grouping/selection test passes; requested-terminal selection and native fixture pending. |
| OM-008 | Canvas actions | Event before reply and event during catalog load | Delay response, publish update, navigate A/B/A | Busy clears for owner; stale reply cannot affect another workspace; catalog converges | corregido sin revalidar | pendiente | F027 source uses independent action epochs and coalesced refresh. Model guard tested; real bridge race pending. |
| OM-009 | Canvas Close/Remove | Confirmation/cancel and changed target/state | Open confirmation then cancel or change fixture state | No immediate mutation; scope/state rechecked on submit | en curso | pendiente | F028 native SHA068fceaf Close confirmation and Cancel match Flutter; semantic Dialog exposed and live Canvas retained. Remove/changed state/final mutations remain pending. |
| OM-010 | Canvas document | Every renderer and immediate/controlled action | Exercise AgentRunHeader, TaskProgress, DecisionRequest, ChangeSummary, FileReferenceList, ValidationResults, RiskSummary, ArtifactCard, Notice and ActionGroup | Exact schemas and actual local navigation/controller effects, not just accepted host request | en curso | pendiente | Ten specialized renderers implemented, 12 directed tests; native decision resolution, Open Search and Files work. SHA068fceaf width/scroll fixed and revalidated with equivalent Flutter captures. Line-wrap/density, remaining action/error/race and release coverage still open. |
...
| KB-030 | openAutomations / Open Automations | Open the automation catalog. | Invoke Mod+Shift+A in Global scope and in terminal focus; close/reopen | fallo | pendiente | Terminal path passes, but active Welcome fails in F013-gpui-active-shortcut. F032 shell-focus correction and dispatch test in source; native revalidation pending. |
...
| AU-002 | Create disabled automation and validate schedule | No accidental autostart or execution | en curso | pendiente | Native SHA cfd92ac6 created Review Form Parity through UI, oneTime2031/existingTab, explicit project, Unicode description, queue4/retries2. Runtime confirms revision1/draft/unapproved/runs[]. Flutter reload reads the same values. Other schedules/targets, invalid inputs and release remain open. |
...
| AU-010 | Project/profile/tag/state/search filters and selected detail | Conjunctive filters with valid visible selection and retained reset controls | corregido sin revalidar | pendiente | F031 typed filter catalog in source; no filter may leave the detail on an unrelated hidden record. Native populated fixtures pending. |
| AU-011 | Schedule variants and validation | Recurring/oneTime, timezone, start/end/max runs persisted without changing unrelated fields | en curso | pendiente | F031 configurable calendars/bounds implemented; round-trip tests pass for both schedules. Native new oneTime2031 form saved and Flutter reads it. Recurring bounds/timezone/error/release variants pending. |
| AU-012 | Target variants and ID pickers | Existing tab, fresh tab and managed workspace with correct scoped IDs | en curso | pendiente | Native existingTab creation chose Alpha project/workspace/Terminal1 by labels and persisted exact IDs. Keyboard menu selection/reopen/Escape works. FreshTab/managedWorkspace/profile loading/errors/long lists remain pending. |
| AU-013 | Metadata/policies/timeouts/retry/circuit/notifications | All configured values survive save/edit/reset, no accidental run or approval | en curso | pendiente |13 automation regressions pass. Native Unicode description, Setup parallel, queue4/retries2 persist and Flutter reads them; Notify remains off, draft unapproved and no runs. Other policies/errors/races/toggle animation/drag/release pending. |
| AU-014 | Edit one-time/existingTab/managedWorkspace definitions | No implicit conversion to recurring UTC or freshTab | en curso | pendiente | B native oneTime/existingTab name edit succeeds with unchanged 2030 UTC date and target IDs, revision2 remains draft and runs[]. Flutter reads the updated record. Managed-workspace/race/release variants pending. |
| AU-015 | Master divider and modal keyboard lifecycle | Drag beyond grip, cancel, close/reopen, no dead shortcuts or background tab traversal | en curso | pendiente | F033 native drag/reverse and F032 cold-Welcome shortcut/reopen now pass. New nested editor modal Escape/outside behavior, focus traversal and release remain pending. |
| AU-016 | Run Now/Pause choice dialogs | No dispatch before user chooses; precheck/draftTest/exactRevision/overlap and active-run policy preserved | bloqueado | pendiente | Source fixed and choice/scope tests pass; Flutter chooser opened and cancelled without Run. Computer Use rejected GPUI Run Now in build30a963be as possible execution. Do not retry via another path; native check requires resolving that safety boundary. No automation executed. |
| AU-017 | Failed save retains draft and visible error | Keep fields during pending/error; no wrong-record completion after close/reopen | en curso | pendiente | Native30a963be duplicate slug fails visibly without closing editor or writing either record. Corrected slug retry succeeds and retains all data, draft revision2/runs[]. Delayed completion/close-reopen and release remain pending. |
