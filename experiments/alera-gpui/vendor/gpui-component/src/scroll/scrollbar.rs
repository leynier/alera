...
impl ScrollbarStateInner {
    fn with_drag_pos(&self, axis: Axis, pos: Point<Pixels>) -> Self {
        ...
        // The first move may arrive in the same event batch as mouse-down.
        // Backdate the limiter so a short drag with only one move is never lost.
...
mod tests {
    ...
    #[test]
    fn starting_drag_allows_the_first_move_immediately() {
        let state = ScrollbarStateInner {
            hovered_axis: None,
            hovered_on_thumb: None,
            dragged_axis: None,
            drag_pos: point(px(0.), px(0.)),
        ...
        .with_drag_pos(Axis::Vertical, point(px(0.), px(12.)));

        assert_eq!(state.dragged_axis, Some(Axis::Vertical));
        assert!(state.last_update.elapsed() >= Duration::from_millis(500));
    }
}
