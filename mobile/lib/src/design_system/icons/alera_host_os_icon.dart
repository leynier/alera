...
// `lib/src/design_system/icons/alera_host_os_icon.dart` because
...
enum HostOs {
  ...
  /// Accepts the spellings the runtime and bootstrap probes emit
  /// (`macos`/`darwin`, `windows`/`win32`/`windows_nt`, `linux`). Anything
  /// else is [unknown] so callers fall back to the generic host glyph.
  static HostOs parse(String? value) {
    return switch (value?.trim().toLowerCase()) {
      'macos' || 'darwin' || 'mac' => HostOs.macos,
      'windows' || 'win32' || 'win' || 'windows_nt' => HostOs.windows,
      'linux' => HostOs.linux,
      _ => HostOs.unknown,
    };
  }
