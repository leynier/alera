...
class const _MobileCodexToolMediaSummary({
  ...
  Widget build(BuildContext context) => Row(
    children: <Widget>[
      ...
      Expanded(
        child: Text(
          '${_mobileToolFieldLabel(value.kind)} - ${value.mimeType} - ${value.byteLength == null ? 'Linked media' : _mobileCodexToolByteSize(value.byteLength!)}',
...
_MobileCodexToolMediaValue? _mobileCodexToolMediaValue(
  ...
  if (directKind != null) {
    ...
    if (retainedByteLength is num) {
      return _MobileCodexToolMediaValue(
        directKind,
        mimeType ?? '$directKind/*',
    ...
    if (linkedSource is String) {
      return _mobileCodexToolMediaFromSource(
        directKind,
        mimeType,
        linkedSource,
      );
    ...
    if (embeddedData is String) {
      return _MobileCodexToolMediaValue(
        directKind,
        mimeType ?? '$directKind/*',
        _mobileCodexBase64DecodedLength(embeddedData),
      );
    }
  }
  ...
  if (embeddedKind != null && blob is String) {
    return _MobileCodexToolMediaValue(
      embeddedKind,
      mimeType!,
      _mobileCodexBase64DecodedLength(blob),
    );
  }
  return null;
}

_MobileCodexToolMediaValue _mobileCodexToolMediaFromSource(
  String kind,
  ...
  return _MobileCodexToolMediaValue(
    ...
    comma < 0 ? 0 : _mobileCodexBase64DecodedLength(source, start: comma + 1),
  );
}

int _mobileCodexBase64DecodedLength(String source, {int start = 0}) {
  var end = source.length;
  while (end > start &&
      _mobileCodexBase64Whitespace(source.codeUnitAt(end - 1))) {
    end -= 1;
  }
  var padding = 0;
...
bool _mobileCodexBase64Whitespace(int codeUnit) =>
    codeUnit == 0x20 ||
    codeUnit == 0x09 ||
    codeUnit == 0x0a ||
    codeUnit == 0x0d;
