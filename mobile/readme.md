...
Run the unified **Cut Release** GitHub Actions workflow to create releases. It detects changes under `mobile/` independently from desktop changes and derives the next patch, minor, or major version from Conventional Commit metadata. Mobile tags use `vX.Y.Z-mobile` for stable releases and `vX.Y.Z-rc.N-mobile` for release candidates, independently from desktop tags. Mobile release notes only include changes that touch `mobile/`, and mobile releases never take the repository's Latest badge (it stays on the desktop release). When mobile changed, the workflow analyzes and tests the app, builds one arm64 APK (`alera-X.Y.Z-android.apk`), verifies SHA-256 checksums, creates a draft GitHub Release, uploads and attests the artifact, and publishes only after verification. That APK is arm64-only so it installs on 16 KB page-size phones. Use its `dry_run` input to preview the release plan without creating builds, commits, tags, or releases.
