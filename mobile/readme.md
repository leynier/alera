...
- **NetBird**: run `alera mobile --json enable --netbird` or pick NetBird in settings. The runtime reads `netbird status --json`, requires a connected NetBird peer, binds the gateway to its NetBird IPv4 (100.64.0.0/10), and emits `ws://<netbird-ip>:<port>` pairing offers. This supports both NetBird Cloud and self-hosted management servers. NetBird is detected, never bundled: install and sign in on both devices, and ensure the peer policy permits the gateway port.
