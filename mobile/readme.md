...
Run the unified **Cut Release** GitHub Actions workflow to create releases. It detects changes under `mobile/` independently from desktop changes and derives the next patch, minor, or major version from Conventional Commit metadata. Mobile tags use `vX.Y.Z-mobile` for stable releases and `vX.Y.Z-rc.N-mobile` for release candidates, independently from desktop tags. Mobile release notes only include changes that touch `mobile/`, and mobile releases never take the repository's Latest badge (it stays on the desktop release). When mobile changed, the workflow analyzes and tests the app, builds an arm64 default APK plus architecture-specific APKs, verifies SHA-256 checksums, creates a draft GitHub Release, uploads and attests every artifact, and publishes only after the complete asset set passes verification. The default `alera-X.Y.Z-android.apk` is arm64-only so it installs on 16 KB page-size phones; 32-bit and x86_64 remain separate assets. Use its `dry_run` input to preview the release plan without creating builds, commits, tags, or releases.
...
Release APKs are signed with the stable upload keystore stored in the `ALERA_ANDROID_*` repository secrets (see `docs/release-trust.md`), and the workflow fails if they are missing. Local `flutter build apk --release --target-platform android-arm64` without `android/key.properties` falls back to the debug key; such APKs cannot update over release-signed installs. Omit `--target-platform` only when you intentionally want a fat APK; that build will not install on 16 KB page-size phones.
