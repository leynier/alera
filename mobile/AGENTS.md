...
- The one-shot build_runner convention from the root `AGENTS.md` applies scoped to this package: after finishing a planned batch of edits that touches generated surfaces, run `dart run build_runner build -d` once from `mobile/`, before formatting, analysis, and tests. Do not run a watcher.
