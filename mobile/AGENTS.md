...
- A restored snapshot is replayed at the size the host says it was written at (`snapshotCols`/`snapshotRows` on the attach and resume payloads), and only then does the view state the phone's own viewport. The snapshot is the raw PTY stream, so it reconstructs its screen only at the geometry that produced it: parsed at a phone's much narrower width, every absolute cursor move and hard wrap lands in the wrong column, which reads as a garbled scrollback under a live screen that looks fine because the running program redrew that part itself. Resizing afterwards turns the difference into a reflow instead. Both fields are additive and MUST NOT bump `aleraMobileProtocolVersion`; a host that omits them replays at whatever size the emulator already had.
- The terminal view stays unmounted while a restore drains, rather than being covered. Mounting it states the phone's viewport, which would resize the emulator away from the size the history is being replayed at, reflowing a prefix and then fighting the rest. It also keeps the replay off screen, which otherwise reads as the terminal scrolling itself from the top of history down to the live screen every time a tab is opened.
- The emulator opts into `reflowWithHiddenCursor`. A hidden cursor in the main buffer is read as a full-screen application that will redraw itself, so a resize truncates every line instead of reflowing it. That holds for the live screen and not for the history above it, and an agent TUI keeps the cursor hidden for as long as it runs.
- A phone attaches without stating a viewport until its terminal has been laid out. The host resizes the live PTY to whatever viewport a phone claims, so a placeholder resizes the running session to a size nobody is looking at and again to the real one a layout later.
