...
- The Rust build cache needs two separate entries per platform, because cargo writes to two target directories. `Swatinem/rust-cache` with `workspaces: rust` covers `rust/target`, which is the `alera-cli` sidecar; cargokit sets `CARGOKIT_TEMP_DIR` under the Flutter build directory, so `alera_native` and `code_forge` land in `build/**/cargokit_build` and need their own `actions/cache` entry. Only `warm-cache.yml` may save that entry, so pull requests restore a shared copy instead of each writing a private one into its own ref scope.
