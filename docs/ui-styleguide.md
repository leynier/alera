...
Settings > Application > Desktop exposes an Experimental Mode switch for every desktop workspace. Classic is the default and retains its split layout. Experimental starts with a chrome-less primary terminal in the center. Tabs dragged from the right panel land in the center with Classic drop zones. The tab strip appears when the center has more than one tab and hides again at one. The workspace row mixes with the center agent while only one agent runs there; two or more center agents nest like Classic. Right-panel agents stay extra child rows and do not change that count. When Experimental Mode is on, New Workspace Tools in that same Desktop group picks which Explorer, Search, Source Control, and Pull Request tabs open, and in what order, for workspaces created after that. Existing workspaces keep their own panel. An empty selection keeps a new panel empty until the user adds a tool.

Experimental uses a splitable right tab workbench for auxiliary terminals, files, previews, diffs, and Explorer, Search, Source Control, and Pull Request tools. Those tools are tabs in that tree: each exists at most once, can sit beside any other tab, and can be split or dragged like a terminal. The Add Tab menu also offers Terminal and any Agent Profiles marked to appear next to New Terminal. Choosing a profile opens a prompt dialog first, with dictation, file attachments, and Skip to launch without a prompt. An empty panel offers those same choices without opening a tool automatically. Each workspace retains its own pane tree, order, and selection in desktop view preferences. Hiding the panel preserves its tabs and returns terminal focus to the primary.
...
- Launch the previewer with `flutter widget-preview start <absolute-project-path>` (the repository root for desktop, or its `mobile` directory). Passing an absolute path also keeps Flutter 3.47's scaffold restart anchored to the application when a pubspec changes. Previews render on Flutter Web, so they cover UI only - terminal runtime, updater, and other native code paths are not previewable and stay out of the design system. Quota previews use the presentational content; the application wrapper supplies its runtime actions.
- The checked-in `.widget_preview/pubspec_overrides.yaml` files preserve the app's terminal forks in generated preview scaffolds. After `flutter widget-preview clean`, restore that file with `git restore -- .widget_preview/pubspec_overrides.yaml` from the corresponding application directory before starting again.
