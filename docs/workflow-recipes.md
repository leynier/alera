...
Personal writes use a transactional compare-and-swap. Omit `expectedRevision` only for a new id; updates require the current revision. A stale or duplicate create fails without changing the stored record. After a successful save, authenticated local clients receive `workflowCatalogChanged` with source and catalog revision. A timed-out save has an uncertain outcome: refresh the recipe before retrying, rather than assuming failure or overwriting a newer edit.

## Settings And Reviewed Export

Settings > Workflows presents the catalog with explicit origins, search and a project workspace selector. Selecting a workspace here does not switch the active workspace. Built-in and Project recipes can be copied to Personal; Personal recipes can be edited with their included role contracts and validated before saving. Editing an existing Personal recipe keeps its identity; use Copy To Personal to choose a new id. A draft survives navigation between Settings sections within the current desktop session. Project entries offer Open File, including invalid recipes that need repair.

After a stale or uncertain save, Review Current Recipe compares the runtime's current Personal version with the retained draft. Matching content confirms the save without repeating it. Otherwise, Keep My Draft adopts only the reviewed catalog revision, leaving Save Personal as a separate explicit write; Use Current Version explicitly replaces the draft. Concurrent updates after review still fail the revision check. Copy recovery looks up the candidate id in the draft, never the source recipe, and an absent candidate keeps create-only semantics.

Export To Project uses the independent additive `workflowRecipeExportV1` capability. `workflows.previewRecipeExport` accepts `workspaceId`, a portable `filename` and `document`, returning the relative destination, previous text, canonical exported text and `expectedDigest`. Preview does not create directories or files. Settings shows the destination and a line diff. `workflows.applyRecipeExport` requires that digest and the same destination/document. The digest binds the workspace instance, path, prior bytes and exported definition. Changes since preview require another review. Export remains local-client-only and does not grant execution or approval permissions.

Writes stay in the bounded Rust catalog pool. New files are published without overwriting a concurrent create. Replacing a reviewed file retains its previous inode in a uniquely named `.export-retained-*.bak` sibling, allowing recovery and preserving edits through open external editor handles. The response and success message report this retained path. Retained files are not recipes and are not cleaned automatically. On a race or interruption, inspect the destination and retained file before retrying; a timed-out write has an uncertain outcome. Filesystems without hard-link support reject publication and retain any displaced original for recovery.
