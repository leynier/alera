...
AI Assist names new agent conversations automatically in terminal tabs and Codex Chat. Desktop and mobile share the title persisted by the runtime. Configure the provider, model, and instructions under **Settings > AI Assist > Agent Titles**. **Auto-Generate Agent Titles** defaults on and follows the main AI Assist switch; turning it off retains existing titles and leaves manual generation available while AI Assist itself remains enabled.
