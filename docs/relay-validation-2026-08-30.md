...
The scoped file-size ratchet passes after extracting relay handlers from `server.rs`. The optional repository-wide scan still flags two unchanged files already exceeding 500 lines in the starting revision: `mobile/lib/src/features/terminal/application/terminal_session_controller.dart` (501) and `mobile/test/host_connection_controller_test.dart` (507). Their baselines were not relaxed.
