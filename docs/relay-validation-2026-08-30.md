...
After this follow-up, the exact global file-size check, desktop formatting and analysis, mobile analysis and all 594 ordinary mobile tests passed locally. Riverpod was regenerated once for the extraction. The controller now has 456 lines and the connection test file has 495 lines. These results preceded publication of the static-check follow-up.

PR mobile job `99206633580` subsequently failed on the integration tree, not the isolated branch: quota-host settings tests added on `main` still overrode the former functional `availableHostsProvider`, while relay discovery now uses the `AvailableHosts` notifier. Synchronizing the exact CI base `dd9008ef` reproduced all four analyzer errors. The fix uses a test notifier for those four overrides without changing production discovery or removing assertions. All seven quota-host settings tests and mobile analysis passed afterward.

The synchronized tree then passed the full mobile suite with 617 tests and the usual three skipped cross-language cases. Desktop/mobile formatting and analysis, the global file-size ratchet and whitespace checks also passed locally. This verifies the combined sources used by CI; no updated remote run has been claimed.

PR coverage job `99208950799` reported 5,531/5,532 maintained domain lines: only the populated `relayStatus` parsing branch was missing. `pr-ready` job `99209506938` failed solely because coverage failed; static, mobile, Rust and the test shards were successful. New repository tests verify that relay health is preserved as an independent snapshot and that missing or malformed health remains compatible with older runtimes. The coverage threshold and exclusions are unchanged.

Validation used the exact CI integration sources, including base `daa76fde`, and Flutter 3.44.8. The affected shard 3 passed 1,043 tests with one usual skip. Combining its fresh coverage with the unchanged successful shards 0, 1 and 2 downloaded from run `33293260310` passed the same four-input gate at 100.00% (5,532/5,532 maintained domain lines). The original artifact set reproduced the failure before replacement. Formatting, the global file-size ratchet and Flutter analysis also passed. Local package metadata was refreshed because it still referenced an SDK directory that had been removed; dependency versions were not changed.

CodeQL alert 39 points to a hard-coded handshake nonce in the `#[cfg(test)]` writer receipt test. That test now generates its keys and 128-bit handshake nonce from secure randomness. It still checks both acknowledged writes and cancelled writes before restart. Production handshake and encryption code are unchanged, and no alert suppression was added. The 25 relay-related Rust tests and Clippy with warnings denied passed; one cross-language fixture remains intentionally ignored in ordinary Rust runs. CodeQL itself must run again after publication to confirm the alert's remote state.
