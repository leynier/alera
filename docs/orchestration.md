...
The built-in registry supports `codex`, `claude`, `copilot`, `cursor`, `agy`, `opencode`, `opencode2`, `pi`, `amp`, and `grok`, with default commands `codex`, `claude`, `copilot`, `cursor-agent`, `agy`, `opencode`, `opencode2`, `pi`, `amp`, and `grok`. `agent-spawn --command` overrides the default without changing the agent type.
