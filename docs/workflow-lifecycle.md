...
An integration in Attention never retries on Start or host restart. Inspect its task and integration workspace, correct the obstruction, then choose Retry Integration and confirm the same retained result. The runtime rechecks the original request, Git resources, receipt and expected SHA; an unresolved issue stays in Attention without overwriting files. A successful retry leaves execution paused until Start Workflow is selected separately.
...
Transient cancellation sweep failures retry on the runtime's background lane after a two-second delay, without starting a second concurrent sweep. A terminal settlement failure is recorded as Attention when storage remains available, and other targets on the page still receive cancellation. Recorded Attention waits for explicit Retry Cancellation; automatic retries only revisit pending work.

Previously reserved integrations settle separately after cancellation. Recovery only inspects existing Git receipts and clean, unchanged owned worktrees; it never prepares or applies another result. An already-applied receipt remains in history, but cancellation creates no task evidence or dependent work. Dirty files, changed identity or an unexpected HEAD remain Attention for inspection and explicit Retry Cancellation. Automatic recovery and restart leave Attention unchanged; only a new sequence-checked cancellation enables another inspection, and replaying a lost response does not retry a later failure. Reviewed cleanup becomes available only after outstanding operations have settled.
