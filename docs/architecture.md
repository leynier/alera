...
Alera can show local Codex, Claude Code, GitHub Copilot, Antigravity, OpenCode, Pi, and Amp status for terminal workspace tabs when the user enables that agent's hook toggle in Settings. Each toggle is default-off because enabling most integrations writes managed hook entries, managed status files, or local runtime overlays. Codex and Claude Code are scoped to Alera-managed runtime homes under the application support directory: Alera mirrors the user's `~/.codex` resources/config into the Codex runtime home, writes Codex status hooks to the runtime `hooks.json`, and injects `CODEX_HOME` only into Alera terminal launches while the Codex toggle is enabled; Claude Code links or copies the user's Claude config resources into its runtime config directory, writes status hooks to the runtime `settings.json`, mirrors macOS Keychain credentials into the runtime-scoped Claude Code service, and injects `CLAUDE_CONFIG_DIR` only into Alera terminal launches while the Claude Code toggle is enabled. OpenCode and Pi are scoped to Alera-managed per-terminal overlays under the application support directory: Alera mirrors the user's OpenCode config or Pi agent directory into an overlay, writes the status plugin/extension inside that overlay, and injects `OPENCODE_CONFIG_DIR` or `PI_CODING_AGENT_DIR` only into Alera terminal launches while the matching toggle is enabled. GitHub Copilot and Antigravity write managed hook entries into `~/.copilot/hooks/alera.json` and `~/.gemini/config/hooks.json` for the `agy` CLI, plus fail-open scripts under `~/.alera/agent-hooks/`. Amp still writes a global managed plugin to `~/.config/amp/plugins/alera-agent-status.ts`; `AMP_CONFIG_DIR` is honored when present. Disabling an agent removes only Alera-managed hook entries or legacy managed files and preserves user-authored hooks/files.
...
Managed hook scripts, managed status files, and overlay plugins read the endpoint file on every invocation/event before posting. This lets a PTY that survived an Alera restart report to the newly opened app receiver once the app is running again. Events emitted while the app is closed are intentionally missed because the receiver is app-local and the terminal host does not buffer hook events. OpenCode and Pi overlays no-op outside Alera because they are exposed only through Alera terminal launch environment. Amp remains a global integration for now, but it no-ops outside Alera because the required `ALERA_TERMINAL_SESSION_ID`, `ALERA_WORKSPACE_ID`, and `ALERA_TAB_ID` metadata is absent.
